...
exports.listAdmin = async ({ page=1, limit=10, filters={} }) => {
  ...
  const items = await Property.find(query).populate('owner', 'name email').skip(skip).limit(limit).sort({ createdAt: -1 });
...
exports.listPublic = async ({ page=1, limit=10, filters={} } = {}) => {
  ...
  const items = await Property.find(baseQuery).populate('owner', 'name').skip(skip).limit(limit).sort({ createdAt: -1 });
  return { items, total, page, limit};
};
